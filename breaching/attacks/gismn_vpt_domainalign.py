...
class SemanticSimilarityBasedLatentCodeAttackerVPT(OptimizationBasedAttacker):
    ...
    def __init__(self, model, loss_fn, cfg_attack, setup=dict(dtype=torch.float, device=torch.device("cpu"))):
        ...
        log.info(f"Initialized GISMN attacker with generator type {self.cfg.generator.type} and num_classes={self.num_classes}.")
    ...
    def _run_trial(self, rec_model, shared_data, labels, stats, trial, initial_data=None, dryrun=False):
        ...
        log.info(f"Initializing generator of type {self.cfg.generator.type} for trial {trial}.")
        if self.cfg.generator.type == "biggan":
            self.netG = BigGAN(self.bigganconfig).to(self.setup["device"])
        ...
        try:
            for iteration in range(self.cfg.optim.max_iterations):
                ...
                with torch.no_grad():
                    ...
                    if self.cfg.generator.type == "biggan":
                        candidate = self.netG(self.noise, cond_labels, truncation=self.truncation)
                        candidate = self._resize_biggan_candidate(candidate)
                    elif self.cfg.generator.type == "styleganxl":
                        candidate = self.netG(self.noise, cond_labels, truncation_psi=self.truncation, noise_mode='random')
                    elif self.cfg.generator.type in ("stylegan2", "stylegan2_ada"):
                        candidate = self.netG(self.noise, cond_labels, truncation_psi=self.truncation, noise_mode='random')
    ...
    def _run_parallel_trials(self, rec_model, shared_data, labels, stats, initial_data, dryrun, num_trials):
        ...
        if self.cfg.generator.type == "biggan":
            ...
            cond_labels = self._make_cond_label(labels, self.setup["device"], netGs[-1].c_dim)
        elif self.cfg.generator.type in ("stylegan2", "stylegan2_ada"):
            cond_labels = self._make_cond_label(labels, self.setup["device"], netGs[-1].c_dim)

        with torch.no_grad():
            best_candidates = [
                self._generate_candidate(netG, noise, cond_labels).detach().clone()
        ...
        try:
            for iteration in range(self.cfg.optim.max_iterations):
                ...
                for netG, noise in zip(netGs, noises):
                    candidate = self._generate_candidate(netG, noise, cond_labels)
    ...
    def _compute_objective(self, netG, noise, cond_labels, labels, rec_model, optimizer, shared_data, iteration):
        def closure():
            optimizer.zero_grad()
            if self.cfg.generator.type == "biggan":
                candidate = netG(noise, cond_labels, truncation=self.truncation)
                candidate = self._resize_biggan_candidate(candidate)
            elif self.cfg.generator.type == "styleganxl":
                candidate = netG(noise, cond_labels, truncation_psi=self.truncation, noise_mode='random')
            elif self.cfg.generator.type in ("stylegan2", "stylegan2_ada"):
                candidate = netG(noise, cond_labels, truncation_psi=self.truncation, noise_mode='random')
