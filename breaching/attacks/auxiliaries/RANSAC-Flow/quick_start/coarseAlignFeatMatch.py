...
class CoarseAlign:
    def __init__(
        ...
        if imageNet : 
            ...
            resNetfeat = models.resnet50()
            featPth = os.path.join(
            ...
            # This checkpoint is trusted (local file), so allow full load fallback.
            try:
                param = torch.load(featPth, weights_only=True)
            except Exception:
                param = torch.load(featPth, weights_only=False)
            state_dict = {k.replace("module.", ""): v for k, v in param['model'].items()}
            state_dict.pop("fc.weight", None)
            state_dict.pop("fc.bias", None)
            msg = 'Loading pretrained model from {}'.format(featPth)
            print(msg)
            resNetfeat.load_state_dict( state_dict , strict=False)
